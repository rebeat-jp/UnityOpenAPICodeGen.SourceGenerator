...
namespace ReBeat.OpenApiCodeGen.SourceGenerator
{
    ...
    internal sealed class GenerationService
    {
        ...
        public IReadOnlyList<GeneratedFile> GenerateFromOpenApiJson(string json, ApiClientGenerateOption option)
        {
            ...
            var source = GenerateApiClient(document, option);
            return new[] { new GeneratedFile($"{option.ApiName}.g.cs", source) };
        }
        ...
        private static string GenerateApiClient(OpenApiDocument document, ApiClientGenerateOption option)
        {
            ...
            foreach (var path in document.Paths)
            {
                foreach (var operation in path.Value.Operations)
                {
                    ...
                    var method = CreateMethod(methodName, operation.Value, option, ref usesCollections);
                    methods.Add(method);
                }
            }
            ...
            ref bool usesCollections)
        {
            ...
            var method = SyntaxFactory.MethodDeclaration(returnType, SyntaxFactory.Identifier(methodName))
            ...
            bool hasReturn)
        {
            ...
            var xml = "/// <summary>" + EscapeXml(summary ?? operationId ?? "Operation") + "</summary>\n";
